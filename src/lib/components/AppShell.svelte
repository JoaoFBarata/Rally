<script lang="ts">
	...
	async function applyNotificationPreference(userId: string, enabled: boolean) {
		const nextKey = `${userId}:${enabled ? 'on' : 'off'}`;
		if (pushPreferenceKey === nextKey) return;
		pushPreferenceKey = nextKey;

		if (!enabled) {
			stopNotifications();
			if (Capacitor.getPlatform() !== 'web') {
				await removePushListeners();
				await removeStoredPushTokenForUser(userId);
			}
					...
					unsubscribeUserDoc = onSnapshot(
						...
						(snapshot) => {
							...
							if (snapshot.exists()) {
								...
								void applyNotificationPreference(user.uid, profile.notificationsEnabled ?? true);
							} else {
								profile = null;
								void applyNotificationPreference(user.uid, true);
							}
...
{:else}
	<div class="min-h-screen bg-white text-black dark:bg-[#161616] dark:text-white font-montserrat">
		<div class="flex min-h-screen min-w-0 overflow-x-clip">
			...
			>
				<div
					class={`mt-12 flex h-12 w-full shrink-0 items-center px-6.75 ${sidebarCollapsed ? 'justify-center' : 'justify-between'}`}
				>
					{#if !sidebarCollapsed}
				...
				<div
					class={`mt-14.25 flex w-full justify-center transition-[padding] duration-300 ease-in-out ${sidebarCollapsed ? 'px-4.75' : 'px-6.75'}`}
				>
					<button
