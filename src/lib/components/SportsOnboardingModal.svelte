<script lang="ts">
	...
	async function saveSports() {
		...
		try {
			...
			error = err instanceof Error ? err.message : 'Could not save your sports.';
		} finally {
			saving = false;
		}
	}
...
>
	<div class="my-auto w-full max-w-3xl overflow-hidden rounded-[2rem] border border-white/70 bg-white shadow-2xl shadow-slate-950/30 dark:border-slate-700 dark:bg-slate-900">
		<div class="bg-gradient-to-br from-blue-600 via-blue-600 to-cyan-500 px-5 py-6 text-white sm:px-8 sm:py-8">
			<p class="text-xs font-black uppercase tracking-[0.24em] text-blue-100">Rally</p>
			<h2 id="sports-onboarding-title" class="mt-2 text-2xl font-black tracking-tight sm:text-4xl">
		...
		<div class="p-4 sm:p-8">
			<div class="grid max-h-[52dvh] grid-cols-2 gap-2 overflow-y-auto pr-1 sm:grid-cols-3 sm:gap-3 lg:grid-cols-4">
				{#each availableSports as sport (sport)}
					<button
						...
						class={`flex min-w-0 items-center gap-2.5 rounded-2xl border p-3 text-left transition duration-200 sm:p-4 ${
							selectedSports.includes(sport)
								? 'border-blue-600 bg-blue-50 text-blue-700 shadow-md shadow-blue-600/10 ring-2 ring-blue-600 dark:bg-blue-950/50 dark:text-blue-200'
					...
					>
						<span class="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white shadow-sm dark:bg-slate-900">
							<img src="/{sport}_icon.png" alt="" class="h-6 w-6 object-contain" />
						</span>
			...
			{#if error}
				<p class="mt-4 rounded-2xl bg-red-50 px-4 py-3 text-sm font-bold text-red-700 dark:bg-red-950/50 dark:text-red-300">{error}</p>
			{/if}

			<div class="mt-5 flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800">
				<p class="text-xs font-bold text-slate-500 dark:text-slate-400">
					{selectedSports.length} {i18n.t('sports')}
				</p>
				<button
					type="button"
					onclick={saveSports}
