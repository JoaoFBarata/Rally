...
<div id="marker" class="w-[calc(var(--marker-scale)*7.1875rem)] select-none" style="--marker-scale: calc({marker_scale}*var(--map-zoom-scale)); --marker-color: {marker_color}">
    <div id="marker-icon-wrapper" class="relative w-full h-[calc(var(--marker-scale)*7.1875rem)]">
        <div id="marker-icon" class="w-full h-full">
            <div id="outer-marker-icon" class="w-full h-full border-[calc(var(--marker-scale)*0.25rem)] border-solid border-(--marker-color) rounded-full bg-white dark:bg-slate-900 flex items-center justify-center overflow-hidden">
                {#if profile_url}
                ...
                {:else}
                    <span class="text-slate-700 dark:text-slate-350 font-black text-[calc(var(--marker-scale)*3.75rem)]">
                        {name_letter}
                    </span>
                {/if}
            </div>
        ...
        <div id="marker-stats-wrapper">
            ...
            {:else}
                <div id="marker-occupation-wrapper" class="absolute bottom-0 left-0 w-[calc(var(--marker-scale)*2.3125rem)] h-[calc(var(--marker-scale)*2.3125rem)] rounded-full p-[calc(var(--marker-scale)*0.1875rem)] bg-linear-115 from-[#ff595eFF] from-50% to-[#00B4D8FF] to-50%">
                    <div class="w-full h-full bg-linear-115 from-[#fdfdfdFF] from-50% to-[#00B4D8FF] to-50% flex flex-row items-center justify-between px-[calc(var(--marker-scale)*0.1875rem)] rounded-full">
                        <div class="mb-[calc(var(--marker-scale)*0.25rem)]">
                            <h3 class="text-[calc(var(--marker-scale)*0.625rem)] text-black font-bold">{n_confirmed_attendees}</h3>
                        </div>
                ...
                <div id="marker-sport-wrapper" class="absolute bottom-0 right-0 w-[calc(var(--marker-scale)*2.8125rem)] h-[calc(var(--marker-scale)*2.8125rem)] rounded-full border-[calc(var(--marker-scale)*0.25rem)] border-solid border-[#0065fdFF] bg-[#fdfdfdFF] flex justify-center items-center">
