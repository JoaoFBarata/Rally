...
<script lang="ts">
	...
	function formatCountryName(countryCode?: string | null) {
		...
		return (
			PROMOTION_COUNTRIES.find((country) => country.code === countryCode)?.label ?? countryCode
		);
	}
...
<main class="mx-auto w-full max-w-6xl px-4 py-4 pb-28 sm:px-5 sm:py-8">
	...
	{:else if error && !targetProfile}
		<section
			class="mt-8 rounded-[2rem] bg-red-50 p-6 font-bold text-red-700 dark:bg-red-950/40 dark:text-red-300"
		>
			{error}
		</section>
	{:else if targetProfile}
		<section class="mt-3 min-w-0 overflow-visible sm:mt-5">
			<div class="flex min-w-0 items-start gap-4 sm:gap-6">
				...
				<div class="min-w-0 flex-1 pt-0.5 sm:pt-1">
					<div class="flex min-w-0 items-center gap-2">
						<h1
							class="line-clamp-2 text-2xl font-black tracking-tight text-slate-950 dark:text-slate-50 sm:text-4xl"
						>
					...
					{#if targetProfile.bio}
						<div class="mt-2 max-w-2xl">
							<p
								class="text-[11px] font-black uppercase tracking-[0.18em] text-slate-400 dark:text-slate-500"
							>
								{i18n.t('bio')}
							</p>
							<p
								class="mt-1 line-clamp-3 text-sm leading-relaxed text-slate-700 dark:text-slate-300 sm:text-base"
							>
								{targetProfile.bio}
							</p>
						</div>
			...
			<div
				class="-mx-1 mt-4 flex gap-2 overflow-x-auto px-1 py-1 sm:mx-0 sm:flex-wrap sm:overflow-visible sm:px-0"
			>
				{#if targetProfile.sports?.length}
					{#each visibleSports as sport}
						<span
							class="shrink-0 rounded-full bg-blue-50 px-3 py-2 text-xs font-black text-blue-700 ring-1 ring-blue-100 dark:bg-blue-950/50 dark:text-blue-200 dark:ring-blue-900/60"
						>
							{formatSport(sport)}
						</span>
					{/each}
				...
				{:else}
					<span
						class="rounded-full bg-slate-100 px-3 py-2 text-xs font-black text-slate-500 dark:bg-slate-800 dark:text-slate-300"
						>{i18n.t('no_sports_yet')}</span
					>
				{/if}
			</div>
			...
			>
				<div>
					<p class="text-lg font-black text-slate-950 dark:text-slate-50 sm:text-2xl">
						{allHostedEvents.length}
					</p>
					<p
						class="text-[11px] font-bold leading-tight text-slate-500 dark:text-slate-400 sm:text-xs"
					>
						{i18n.t('events_hosted')}
					</p>
				</div>
				<div>
					<p class="text-lg font-black text-slate-950 dark:text-slate-50 sm:text-2xl">
						{allParticipatedEvents.length}
					</p>
					<p
						class="text-[11px] font-bold leading-tight text-slate-500 dark:text-slate-400 sm:text-xs"
					>
						{i18n.t('events_joined')}
					</p>
				</div>
				<div>
					<p class="text-lg font-black text-slate-950 dark:text-slate-50 sm:text-2xl">
						{targetProfile.rallyPointsTotal ?? 0}
					</p>
					<p
						class="text-[11px] font-bold leading-tight text-slate-500 dark:text-slate-400 sm:text-xs"
					>
			...
			<div class="mt-4 grid grid-cols-2 gap-2 sm:mt-5 sm:flex sm:flex-wrap">
				{#if isOrganizationViewer}
					<span
						class="col-span-2 rounded-2xl bg-slate-100 px-4 py-3 text-center text-sm font-black text-slate-500 dark:bg-slate-800 dark:text-slate-400 sm:col-span-1"
					>
			...
			<section class="min-w-0 max-w-full space-y-5 overflow-visible">
				{#if !hidesPrivateDetails && mutualFriends.length > 0}
					...
					>
						<div class="flex items-center justify-between gap-3">
							...
							<span
								class="rounded-full bg-blue-50 px-3 py-1 text-xs font-black text-blue-700 dark:bg-blue-950 dark:text-blue-300"
							>
						...
						<div class="mt-4 flex items-center gap-3">
							<div class="flex -space-x-3">
								{#each mutualFriends.slice(0, 4) as friend (friend.id)}
									<a
										...
										aria-label={i18n.t('view_profile_aria', { name: friend.displayName })}
									>
										<UserAvatar
								...
								{#if mutualFriends.length > 4}
									<span
										class="grid h-14 w-14 place-items-center rounded-full bg-slate-100 text-sm font-black text-blue-600 ring-2 ring-white dark:bg-slate-800 dark:text-blue-300 dark:ring-slate-900"
									>
							...
							<p class="min-w-0 text-sm font-bold text-slate-500 dark:text-slate-400">
								{i18n.t('mutual_friends_summary', {
									name: targetProfile.displayName,
									count: mutualFriends.length,
									friends: i18n.t(
										mutualFriends.length === 1 ? 'mutual_friend' : 'mutual_friends_plural'
									)
								})}
							</p>
						</div>
				...
				{#if !hidesEventActivity && liveProfileEvents.length > 0}
					<section>
						<div class="mb-3">
							...
							<h2 class="mt-1 text-lg font-black text-slate-950 dark:text-slate-50 sm:text-2xl">
				...
				{#if !hidesEventActivity && upcomingProfileEvents.length > 0}
					<section>
						<div class="flex items-end justify-between gap-3">
							<div>
								<h2 class="text-lg font-black text-slate-950 dark:text-slate-50 sm:text-2xl">
									{i18n.t('upcoming_events_profile')}
								</h2>
								<p class="text-sm font-bold text-slate-500 dark:text-slate-400">
									{i18n.t('upcoming_events_profile_sub')}
								</p>
							</div>
							{#if allUpcomingProfileEvents.length > 3}
								...
								>
									{showAllUpcomingEvents
										? i18n.t('show_less')
										: `${i18n.t('view_all')} (${allUpcomingProfileEvents.length})`}
								</button>
							{/if}
						</div>
						...
						<div class="-mx-1 mt-3 space-y-3 px-1">
							{#each upcomingProfileEvents as event (event.id)}
								...
								>
									<div
										class="relative h-24 w-24 shrink-0 overflow-hidden rounded-2xl bg-slate-100 dark:bg-slate-800 sm:h-28 sm:w-36"
									>
										<img
										...
										/>
										<div
											class="absolute left-2 top-2 rounded-xl bg-blue-600 px-2 py-1 text-center text-white shadow-lg shadow-blue-950/20"
										>
									...
									<div class="min-w-0 flex-1 py-0.5">
										<div class="flex min-w-0 items-center gap-2">
											<p
												class="truncate text-xs font-black uppercase tracking-wide text-blue-600 dark:text-blue-300"
											>
												{formatSport(event.sport)}
											</p>
											{#if event.status}
												<span
													class="shrink-0 rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-black capitalize text-slate-500 dark:bg-slate-800 dark:text-slate-300"
												>
										...
										<h3
											class="mt-1 line-clamp-1 text-lg font-black text-slate-950 dark:text-slate-50 sm:text-xl"
										>
											{event.title}
										</h3>
										<p
											class="mt-1 line-clamp-1 text-xs font-bold text-slate-500 dark:text-slate-400 sm:text-sm"
										>
										...
										<div class="mt-3 flex min-w-0 items-center gap-2 text-xs font-black">
											<span
												class="shrink-0 rounded-full bg-blue-50 px-3 py-1 text-blue-700 dark:bg-blue-950 dark:text-blue-300"
											>
												{formatProfileEventCapacity(event)}
											</span>
											<span
												class="min-w-0 truncate rounded-full bg-slate-100 px-3 py-1 text-slate-600 dark:bg-slate-800 dark:text-slate-300"
											>
									...
									<div
										class="hidden shrink-0 place-items-center text-slate-300 transition group-hover:text-blue-600 sm:grid"
									>
										<svg
											xmlns="http://www.w3.org/2000/svg"
											class="h-6 w-6"
					...
					<section>
						<div class="flex items-center justify-between gap-3">
							...
							{#if allRecentProfileEvents.length > 3}
								...
								>
									{showAllRecentEvents
										? i18n.t('show_less')
										: `${i18n.t('view_all')} (${allRecentProfileEvents.length})`}
								</button>
							{/if}
						</div>
						...
						<div class="-mx-1 mt-3 space-y-3 px-1">
							{#each recentProfileEvents as event (event.id)}
								<a
									href={resolve(`/events/${event.id}`)}
									class="group flex items-center gap-3 rounded-[1.5rem] bg-gradient-to-r from-red-50/80 to-white p-3 shadow-sm shadow-red-100/60 ring-1 ring-red-200/70 transition hover:-translate-y-0.5 hover:shadow-md hover:shadow-red-100 dark:from-red-950/25 dark:to-slate-900 dark:shadow-none dark:ring-red-900/40 dark:hover:from-red-950/35"
								>
									<img
										src={getEventImage(event)}
										alt=""
									...
									<div class="min-w-0 flex-1">
										<p class="line-clamp-1 text-sm font-bold text-slate-500 dark:text-slate-400">
											{targetProfile.displayName}
											{getRecentActivityText(event)}
										</p>
										<h3 class="line-clamp-1 text-base font-black text-slate-950 dark:text-slate-50">
											{event.title}
										</h3>
										<p
											class="mt-0.5 line-clamp-1 text-xs font-bold text-slate-500 dark:text-slate-400"
										>
											{formatEventSchedule(event)}
										</p>
									</div>
				...
				{#if !hidesEventActivity && upcomingProfileEvents.length === 0 && recentProfileEvents.length === 0}
					<section class="rounded-[2rem] bg-white p-8 text-center shadow-sm dark:bg-slate-900">
						...
						<p class="mt-3 font-black text-slate-950 dark:text-slate-50">
							{i18n.t('no_public_events')}
						</p>
						<p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
							{i18n.t('no_public_events_sub')}
						</p>
					</section>
				{/if}
			</section>
		...
		{#if success}
			<p
				class="mt-4 rounded-2xl bg-emerald-50 px-4 py-3 text-sm font-bold text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300"
			>
				{success}
			</p>
		{/if}
			...
			>
				...
				>
					<div class="flex flex-col items-center">
						<div
							class="flex h-14 w-14 items-center justify-center rounded-full bg-red-100 text-red-600 dark:bg-red-950/40 dark:text-red-400"
						>
							<svg
								xmlns="http://www.w3.org/2000/svg"
								fill="none"
						...
						<h2
							...
							class="mt-4 text-xl font-black text-slate-950 dark:text-slate-50"
						>
