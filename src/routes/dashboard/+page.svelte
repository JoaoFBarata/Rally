	...
	<section class="mx-auto max-w-6xl px-5 py-6">
		<header class="mb-6 flex items-start justify-between gap-4">
			<div>
				...
				<h1 class="mt-2 text-3xl font-black text-slate-950 dark:text-slate-50">Dashboard</h1>
		...
		<section class="grid gap-6 lg:grid-cols-[1.4fr_0.8fr]">
			...
			>
				<div class="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
					<div>
						<p class="text-sm font-black uppercase tracking-[0.25em] text-blue-100">Rally home</p>

						<h2 class="mt-3 text-3xl font-black">Organize your next game</h2>

						<p class="mt-3 max-w-xl text-sm leading-6 text-blue-50">
							Create events, invite friends, manage your groups and keep every sports plan in one
							place.
						</p>
					</div>
		...
		<section class="mt-6 grid gap-6 md:grid-cols-3">
			...
			>
				...
				<p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Events created by you</p>
			</div>
			...
			>
				...
				<p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Open or upcoming plans</p>
			</div>
			...
			>
				...
				<p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Pending invites</p>
			</div>
		</section>
		...
		<section class="mt-10 grid gap-6 lg:grid-cols-[1.4fr_0.8fr]">
			<div class="space-y-6">
				...
				>
					<div class="mb-5 flex items-center justify-between">
						<h2 class="text-xl font-black text-slate-950 dark:text-slate-50">My events</h2>
					...
					{#if events.length === 0}
						...
						>
							<p class="text-slate-500 dark:text-slate-400">You have not created any events yet.</p>
				...
				>
					<div class="mb-5 flex items-center justify-between">
						<h2 class="text-xl font-black text-slate-950 dark:text-slate-50">Events I joined</h2>
					...
					{#if joinedEvents.length === 0}
						<div
							class="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center dark:border-slate-700 dark:bg-slate-800"
						>
							<p class="text-slate-500 dark:text-slate-400">You have not joined any events yet.</p>
			...
			<div class="space-y-6">
				...
				<div
					class="rounded-4xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/70 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none"
				>
					<h2 class="text-xl font-black text-slate-950 dark:text-slate-50">Next event</h2>

					{#if nextEvent}
						<div class="mt-5 rounded-3xl bg-slate-50 p-5 dark:bg-slate-800">
							<p
								class="text-sm font-black uppercase tracking-[0.2em] text-blue-600 dark:text-blue-400"
							>
								{nextEvent.sport}
							</p>
					...
					{:else}
						<p class="mt-4 text-sm text-slate-500 dark:text-slate-400">No upcoming events yet.</p>
					{/if}
				</div>
				...
				<div
					class="rounded-4xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/70 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none"
				>
					<h2 class="text-xl font-black text-slate-950 dark:text-slate-50">Rally stats</h2>
					...
					<div class="mt-5 grid grid-cols-2 gap-3">
						<div class="rounded-2xl bg-slate-50 p-4 dark:bg-slate-800">
							...
							<p class="mt-1 text-xs font-bold text-slate-500 dark:text-slate-400">total players</p>
						</div>
						...
						<div class="rounded-2xl bg-slate-50 p-4 dark:bg-slate-800">
							...
							<p class="mt-1 text-xs font-bold text-slate-500 dark:text-slate-400">sports</p>
						</div>
					</div>
				</div>
			</div>
