	...
	onMount(() => {
		const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
			...
			try {
				...
				for (const e of createdEvents) eventsById.set(e.id, e);
				for (const e of participantEvents) eventsById.set(e.id, e);

				allUserEvents = sortEventsByStartDate(Array.from(eventsById.values()));
				events = allUserEvents.filter((e) => e.creatorId === currentUser.uid);
				joinedEvents = allUserEvents.filter(
					(e) => e.creatorId !== currentUser.uid && e.participantIds.includes(currentUser.uid)
				);
...
{:else}
	<div class="mx-auto max-w-6xl px-5 py-8">
		...
		{#if error}
			<div class="mb-6 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-medium text-red-700 dark:border-red-900/60 dark:bg-red-950/40 dark:text-red-300">
				{error}
			</div>
		{/if}
			...
			<div class="min-w-0 space-y-5">
				...
				{#if nextEvent}
					...
					>
						<div class="flex items-center justify-between gap-2">
							<span class="text-xs font-bold uppercase tracking-widest text-blue-500 dark:text-blue-400">
								Next up
							</span>
							<span class="rounded-full bg-blue-100 px-3 py-1 text-xs font-bold uppercase tracking-wide text-blue-700 dark:bg-blue-900/50 dark:text-blue-300">
						...
						<div class="mt-3 flex flex-wrap gap-x-5 gap-y-1.5 text-sm text-slate-500 dark:text-slate-400">
						...
						<div class="mt-4 flex items-center gap-3">
							<div class="h-1.5 flex-1 overflow-hidden rounded-full bg-blue-200 dark:bg-blue-900/60">
								<div
									class="h-1.5 rounded-full bg-blue-500 dark:bg-blue-400"
									style="width: {Math.round((nextEvent.participantIds.length / nextEvent.maxParticipants) * 100)}%"
								></div>
							</div>
				...
				<div>
					...
					{:else}
						{#if joinedEvents.length === 0}
							<div class="rounded-2xl border border-dashed border-slate-200 bg-white p-8 text-center dark:border-slate-700 dark:bg-slate-900">
								<p class="text-sm text-slate-500 dark:text-slate-400">You haven't joined any events yet.</p>
								<a
									href={resolve('/explore')}
									class="mt-3 inline-flex rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-bold text-white transition hover:bg-blue-700"
								>
			...
			<div class="space-y-4">
				...
				{#if pendingInvites.length > 0}
					...
					>
						...
						<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4 shrink-0 text-slate-400" aria-hidden="true">
				...
				<div class="grid grid-cols-2 gap-2">
					...
					>
						<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-5 w-5 text-slate-400" aria-hidden="true">
